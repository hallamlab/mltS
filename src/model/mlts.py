...
class mltS:
    ...
    def __optimize_global(self, X, y, X_trust, y_trust, pi, learning_rate, model_idx, batch_idx, current_progress,
        ...
        count = 1
        current_progress += batch_idx * (model_idx + 1) * len(labels) * max_inner_iter
    ...
    def __batch_backward(self, X, y, y_Bag, X_trust, y_trust, pi, L, learning_rate, samples_idx,
        ...
        current_progress = 0
        subsamples_size = int(np.ceil(len(samples_idx) * self.subsample_input_size))
        tmp_idx = samples_idx
        if len(samples_idx) >= subsamples_size:
    ...
    def __predict(self, X, model_idx, batch_idx, pred_bags, pred_labels, meta_predict, source_model_idx,
                  current_progress,
                  total_progress):
        if pred_labels:
            if not self.label_uncertainty_type == "factorize":
